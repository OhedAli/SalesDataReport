...
class DashboardController extends Controller
{
    public function index(Request $request){
        ...
        $result['weekly_details'] = Saleslogs::select('salesman', 'team', Saleslogs::raw('count(salesman) as sales_count '))
                                    ->whereBetween('create_at',[$lastweek,$todayDate_end])
    ...
    public function salesman_details(Request $request, $name)
    {
        ...
        date_default_timezone_set("America/Chicago");
            ...
            $result['todaycount'] = Saleslogs::whereBetween('create_at',[$todayDate_start, $todayDate_end])->where('salesman',$sm_name)->count();
            $result['yesterdaycount'] = Saleslogs::whereBetween('create_at',[$yesterdayDate_start, $todayDate_start])->where('salesman',$sm_name)->count();
            $result['dailydata'] = $this->FlagSighCheck($result['todaycount'], $result['yesterdaycount']);
        
            $result['weeklycount'] = Saleslogs::whereBetween('create_at',[$lastweek,$todayDate_end])->where('salesman',$sm_name)->count();
            $result['Secondweeklycount'] = Saleslogs::whereBetween('create_at',[$Secondlastweek,$lastweek])->where('salesman',$sm_name)->count();
            $result['weeklydata'] = $this->FlagSighCheck($result['weeklycount'], $result['Secondweeklycount']);
        
            $result['monthlycount'] = Saleslogs::whereBetween('create_at',[$lastmonth,$todayDate_end])->where('salesman',$sm_name)->count();
            $result['Secondmonthlycount'] = Saleslogs::whereBetween('create_at',[$Secondlastmonth,$lastmonth])->where('salesman',$sm_name)->count();
            $result['monthlydata'] = $this->FlagSighCheck($result['monthlycount'], $result['Secondmonthlycount']);
